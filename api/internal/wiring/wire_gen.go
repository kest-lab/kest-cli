...
func InitApplication() (*app.Application, error) {
	...
	runRepository := run.NewRepository(db)
	runnerRunner := runner.New()
	runService := run.NewService(runRepository, runnerRunner)
	runHandler := run.NewHandler(requestService, runService, workspaceService)
